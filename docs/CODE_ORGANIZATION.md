...
└── services/                       # ビジネスロジック
  ...
  └── skillEffectService.ts       # スキル効果ロジック
```
...
export const UNIT_MEMBERS = { 'Cerise Bouquet': [...], ... } as const;
```
...
#### `services/skillEffectService.ts`
...
| `constants/skillEffects.ts` → get\*           | `services/skillEffectService.ts` | ビジネスロジック |
