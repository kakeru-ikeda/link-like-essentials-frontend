  ...
  return (
    <div className="space-y-2">
      <div
        ...
        style={{
          backgroundColor: getCharacterBackgroundColor(centerCard.characterName.join('＆'), 0.25),
        }}
      >
        <div className="flex items-center gap-2 mb-1">
          <span className="text-sm font-bold text-gray-900">
            スペシャルアピール: {centerCard.specialAppeal.name}
          </span>
          {centerCard.specialAppeal.ap && (
            <span className="px-2 py-1 bg-blue-100 text-blue-700 text-xs font-semibold rounded whitespace-nowrap">
              AP {centerCard.specialAppeal.ap}
            </span>
          )}
        </div>
        {centerCard.specialAppeal.effect && (
