  ...
  if (characterName.includes('＆')) {
    for (const [gen, members] of Object.entries(GENERATION_MEMBERS)) {
      const allMembersIncluded = (members as readonly string[]).every(
        (member) => characterName.includes(member)
      );
...
export function canPlaceCardInSlot(
  ...
  if (
    cardGeneration === GENERATION.TERM_102 &&
    cardInfo.characterName.includes('＆')
  ) {
    const slotGeneration = getCharacterGeneration(slotCharacter);
  ...
  // 103期生＆カード（花帆＆さやか＆瑠璃乃）
  if (
    cardGeneration === GENERATION.TERM_103 &&
    cardInfo.characterName.includes('＆')
  ) {
    const slotGeneration = getCharacterGeneration(slotCharacter);
    ...
    return {
      allowed: false,
      reason: '103期生＆カードはすべての期のサイドカードにのみ配置できます',
    };
  }
  ...
  if (cardUnit === 'Edel Note' && cardInfo.characterName.includes('＆')) {
    ...
    if (
      deckType &&
      allowedDeckTypes.includes(deckType) &&
      slotType === 'side'
    ) {
      return { allowed: true };
    }
    ...
    return {
      allowed: false,
      reason:
        'Edeliedは102〜104期またはEdel Noteのサイドカードにのみ配置できます',
    };
  }
  ...
  if (
    cardInfo.characterName === '安養寺姫芽' &&
    cardInfo.cardName === 'IcHiGo milK love'
  ) {
    // スロットキャラクターがPRINCEε>ε>メンバー（安養寺姫芽、日野下花帆、百生吟子、桂城泉）かチェック
    const princeMembers = UNIT_MEMBERS['PRINCEε>ε>'];
    const isPrinceSlot = (princeMembers as readonly string[]).includes(
      slotCharacter
    );
